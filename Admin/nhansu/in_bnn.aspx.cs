...
public partial class Admin_nhansu_in_nnn : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void btnThemMoi_Click(object sender, EventArgs e)
    {
        ...
        if ((txtbactrongngach.Text == "") || (txthesothamnien.Text == ""))
        {
            Label5.Text = "Bạn chưa nhập đúng thông tin";
            return;
        }
        //them moi
        //byte donvi_ID = Convert.ToByte(dlChuyende.SelectedValue);
        ...
        sql_com.Parameters.Add(new SqlParameter("@IDMANGACH", SqlDbType.Int)).Value = mavienchuc_ID;
        sql_com.Parameters.Add(new SqlParameter("@IDHESOLUONG", SqlDbType.Int)).Value = hesoluong_ID;
        sql_com.Parameters.Add(new SqlParameter("@BACTRONGNGACH", SqlDbType.Int)).Value =int.Parse(txtbactrongngach.Text);
        sql_com.Parameters.Add(new SqlParameter("@HESOTHAMNIEN", SqlDbType.Int)).Value = int.Parse(txthesothamnien.Text);
        SqlParameter param = new SqlParameter("@total", SqlDbType.Int);
        param.Direction = ParameterDirection.Output;
        sql_com.Parameters.Add(param);
        sql_com.ExecuteNonQuery();
        ...
        if (total == 0)
        {
            ...
            //TextBox1.Visible = true;
            Label5.Text = "Nhân viên này có rồi hoặc dữ liệu nhập sai";
        }
        catch
        {
