...
public partial class Admin_congvan_cvdi : System.Web.UI.Page
{
    ...
    private void Initiazlie()
    {
        ...
        dtsv.FillDropDownListSQL(dlMacv, "select * from MaCongvandi", "MaCongvandi", "ID");
        ...
        //Loai cong van den
        dtsv.FillDropDownListSQL(ddlLoaiCV, "select * from Loaicongvandi", "Tenloai", "Maloai");

        dtsv.FillGridViewSQL(GridView1, "select * from Congvandi");
    ...
    {
        dtsv.LoadGridView(GridView1,"GetCongvandentheoLoaicongvanadmin",dtsv.CreateParameter("@Loai_ID",ddlLoaiCV.SelectedValue));
    }
    ...
    {
        dtsv.LoadGridView(GridView1, "GetCongvandentheoCoquanbhadmin", dtsv.CreateParameter("@Cqbh_ID", ddlCQBH.SelectedValue));
    ...
    {
        dtsv.LoadGridView(GridView1, "GetCongvandentheoMacongvandenadmin", dtsv.CreateParameter("@Macv_ID", dlMacv.SelectedValue));
    ...
    {
        // Them
        Response.Redirect("nhapcvdi.aspx");
    }
        ...
        try
        {
            ...
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                ...
                if (cbitem.Checked)
                {
                    ...
                    string tenFile=dtsv.ExcuteScalary("select Fileupload from Congvandi where ID="+ID).ToString();
                    string path=Server.MapPath("~/Files/congvan/congvandi/"+tenFile);
                    if(File.Exists(path))
                    File.Delete(path);

                    // thuc hien xoa trong co so du lieu
                    //SqlParameter param = new SqlParameter("@Congvanden_ID", ID);
                    tb = dtsv.ExecuteNonQuery("detele from Congvandi where ID="+ID);
                }
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            dtsv.LoadGridView(GridView1, "Timtungaydenngayadmincongvandi", param);
