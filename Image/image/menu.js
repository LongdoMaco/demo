...
function domMenu_activate(in_containerId)
{
    ...
    for (var index = 1; index <= numSiblings; index++) {
        ...
        if (index == 1 || settings.items['axis'] == 'vertical') {
            ...
            rootMenuTableRow.style.backgroundImage = 'url(../image/ten.gif)';
            rootMenuTableRow.style.backgroundRepeat = 'no-repeat';
            rootMenuTableRow.style.backgroundPosition = 'left center';
            // add appropriate padding to fit the arrow
            rootMenuTableRow.style.paddingRight = '12px';
        }
...
if (in_parentElement.data.hasItem('subMenu')) {
    ...
    for (var index = 1; index <= numSiblings; index++) {
        ...
        if (menuTableCell.data.items['numChildren']) {
            ...
            element.style.backgroundImage = 'url(../image/arrow.gif)';
