...
namespace WebNT.ThuTinDienTu
{
	...
	public class ReceiveMail
	{
		...
		/// <returns>
		/// </returns>
		public bool ReadMessage(MailHeader mailHeader)
		{
			...
			while(true)
			{
				...
				if(readHeder)
				{
					...
					{
						btemp = System.Text.Encoding.Unicode.GetBytes(sTemp.ToCharArray());
						temp = System.Text.Encoding.Unicode.GetString(btemp);
						sBody.Append(sTemp);
						continue;
					} 
					...
					{
						MailReceive.contentType = key.Remove(0,14);
					}
					else if(key.StartsWith("Content-Transfer-Encoding: "))
					{
						MailReceive.contentTransferEncoding = key.Remove(0,27);
					}
				}
				else if(sTemp.StartsWith(delimiter))
				{
					MailReceive.message = sBody.ToString();
					mailHeader.messages.Add(MailReceive);
				...
				{
					btemp = System.Text.Encoding.BigEndianUnicode.GetBytes(sTemp.ToCharArray());
					temp = System.Text.Encoding.UTF8.GetString(btemp);
					sBody.Append(temp);
					sBody.Append("\r\n");
				}
			}
		...
		bool SetMailHeader(MailHeader mailHeader, string key)
		{
			...
			{
				mailHeader.from = key.Remove(0,6).Replace("\"", "");
			}
